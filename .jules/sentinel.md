...
## 2026-05-06 - Missing Referrer-Policy
**Vulnerability:** A static site with multiple external links missing a Referrer-Policy header could inadvertently leak sensitive path information or parameters to cross-origin destinations when users click these links.
**Learning:** Like CSP, missing HTTP security headers like Referrer-Policy can be implemented as `<meta>` tags. Providing an explicit default like `strict-origin-when-cross-origin` sets a secure baseline and prevents the leakage of full paths (which may contain session IDs or other sensitive data, if they were used, although this site is static) and protects user privacy when navigating away.
**Prevention:** Include `<meta name="referrer" content="strict-origin-when-cross-origin">` in all `<head>` sections of static sites.
