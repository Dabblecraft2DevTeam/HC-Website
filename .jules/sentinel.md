...
## 2026-05-05 - Frame-Busting DOMException Leakage
**Vulnerability:** Implementing hide-first anti-clickjacking via frame-busting (`window.top.location = window.self.location`) without a `try...catch` block exposes the application to unhandled `DOMException`s. If an attacker embeds the site in an iframe using `sandbox="allow-scripts"` but omits `allow-top-navigation`, the browser blocks the navigation and throws an error, potentially leaking stack traces and internal execution context.
**Learning:** Security mechanisms like frame-busting can themselves become vectors for information disclosure if they fail ungracefully. Restrictive environments (like sandboxed iframes) can purposefully trigger these errors.
**Prevention:** Always wrap frame-busting logic or any cross-origin window manipulation in a `try...catch` block to handle exceptions silently and prevent information leakage.
