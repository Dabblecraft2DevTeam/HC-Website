...
## 2026-05-18 - Frame-Busting Exception Leak
**Vulnerability:** The JavaScript frame-busting logic (`window.top.location = window.self.location`) was not wrapped in a `try...catch` block. When embedded in restrictive iframes without `allow-top-navigation`, this assignment throws an unhandled `DOMException`, which can leak stack traces and internal execution context to the console or potential attackers.
**Learning:** Security mitigations must themselves fail securely. An unhandled exception in an anti-clickjacking defense can provide an attacker with fingerprintable information about the execution environment or application state.
**Prevention:** Always wrap frame-busting logic or top-level navigation assignments in a `try...catch` block to gracefully handle exceptions in restrictive sandbox environments.
