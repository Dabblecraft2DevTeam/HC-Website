...
## 2026-10-25 - Meta CSP Implicit Resource Allowances
**Vulnerability:** A Content-Security-Policy that defines `default-src 'self'` implicitly allows iframes, child contexts, and web workers from the same origin, even if the static application does not utilize these features. If an attacker manages to compromise a same-origin resource or inject code, they could leverage these allowed contexts to further exploit the application or bypass other restrictions.
**Learning:** For static sites, default CSP directives often leave unnecessary attack surface exposed. `default-src 'self'` should be viewed as a baseline, but specific resource types (like frames, children, and workers) that are completely unused by the application should be explicitly restricted to `'none'` to adhere to the principle of least privilege.
**Prevention:** Always conduct a defense-in-depth review of the CSP. Explicitly set `frame-src 'none'`, `child-src 'none'`, and `worker-src 'none'` (or similarly restrictive directives for other unused resources) in the CSP meta tag to ensure that even if `default-src` is `'self'`, unused execution contexts cannot be spawned.
