...
## 2024-05-18 - Unhandled Frame-Busting Exception Leak
**Vulnerability:** The frame-busting script (`window.top.location = window.self.location`) throws an unhandled `DOMException` when executed within a restrictive sandbox that lacks `allow-top-navigation`. This unhandled exception leaks the stack trace and internal execution context to the browser console or potentially to the parent window, providing an attacker with valuable information for further exploitation.
**Learning:** Security-focused DOM manipulations, like frame-busting, must account for restrictive environments where such actions are blocked. Unhandled exceptions in these scenarios inadvertently expose internal application details.
**Prevention:** Always wrap frame-busting logic (and similar security-sensitive DOM manipulations) in a `try...catch` block. Ensure the `catch` block handles the error gracefully without exposing sensitive stack trace information.
