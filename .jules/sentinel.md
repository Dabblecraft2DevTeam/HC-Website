...
## 2026-10-25 - Defense in Depth with CSP
**Vulnerability:** While `frame-ancestors` is unsupported in `<meta>` Content-Security-Policy tags (preventing direct clickjacking mitigation via CSP without HTTP headers), the existing policy lacked restrictions on what the page itself could embed or execute.
**Learning:** Even if a static site uses JavaScript/CSS for clickjacking defense because `<meta>` tags ignore `frame-ancestors`, it should still enforce strict limits on outgoing embedded contexts.
**Prevention:** As a defense-in-depth measure, explicitly set `frame-src 'none'`, `child-src 'none'`, and `worker-src 'none'` in the `<meta>` CSP tags to restrict the application from embedding iframes, creating child contexts, or spawning web workers.
