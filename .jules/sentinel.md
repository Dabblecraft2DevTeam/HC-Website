...
## 2026-05-15 - Unhandled Exception Leak in Restrictive Sandboxes
**Vulnerability:** The JavaScript frame-busting logic (`window.top.location = window.self.location`) would throw an unhandled `DOMException` when the site was embedded within a restrictive iframe that lacked the `allow-top-navigation` permission. This unhandled exception could leak internal execution context and stack traces.
**Learning:** Even defensive scripts can become a source of leakage when they assume unrestricted environments. It's crucial to account for execution within hostile or highly constrained contexts where seemingly basic operations might fail.
**Prevention:** Always wrap frame-busting logic (and other operations reliant on cross-origin properties) in a `try...catch` block to safely handle and silence `DOMException`s, preventing stack traces from leaking when the operation is blocked.
