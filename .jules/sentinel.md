...
## 2024-05-25 - Content Security Policy Defense-in-Depth for Static Sites
**Vulnerability:** The application relied on a JavaScript-based framebusting script to prevent clickjacking but lacked defense-in-depth CSP directives because `frame-ancestors` is unsupported in `<meta>` tags. Additionally, it did not restrict child contexts or web workers.
**Learning:** For static sites deployed without HTTP headers where `<meta>` tags are used for CSP, `frame-ancestors` is ignored by the browser. To compensate for this and provide defense-in-depth against malicious scripts spawning processes or embedding the application, directives like `frame-src 'none'`, `child-src 'none'`, and `worker-src 'none'` can be appended to the `<meta>` tag to limit the application's capabilities, reducing the risk of exploitation if XSS occurs.
**Prevention:** When evaluating the security posture of statically hosted sites using `<meta>` CSPs, ensure that alongside primary defenses (like anti-clickjack CSS/JS patterns), strict limits on the application's own capabilities (`frame-src`, `worker-src`, `object-src`) are explicitly defined.
