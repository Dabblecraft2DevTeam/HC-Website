...
## 2026-05-11 - Leaking Context via Unhandled DOMException in Restrictive Iframes
**Vulnerability:** Frame-busting script `window.top.location = window.self.location` is vulnerable to throwing an unhandled `DOMException` when embedded in an iframe without `allow-top-navigation` sandbox attribute. The unhandled exception can leak stack traces or internal execution context.
**Learning:** Even defensive scripts can become security vulnerabilities (information disclosure/context leakage) if they do not anticipate strict browser protections or sandbox restrictions on operations like cross-origin navigation or top-level navigation access.
**Prevention:** When implementing or modifying frame-busting logic or accessing restricted `window` properties cross-origin or cross-frame, always wrap it in a `try...catch` block to gracefully handle security exceptions and prevent context leakage.
