...
## 2026-05-09 - Unhandled Exceptions in Framebusting
**Vulnerability:** Framebusting scripts (`window.top.location = window.self.location;`) executing inside sandboxed iframes without `allow-top-navigation` throw `DOMException`s. If not caught, these exceptions are printed to the console, potentially exposing sensitive stack traces and execution context details to an attacker manipulating the iframe environment.
**Learning:** Any browser API interacting with cross-origin or top-level window contexts can throw security exceptions when embedded in restrictive sandboxes. These exceptions must be caught to prevent internal application context leakage.
**Prevention:** Wrap context-sensitive scripts, such as framebusting logic, in `try...catch` blocks and log generic, sanitized error messages rather than raw exception objects.
