...
## 2024-05-14 - Information Leak in Frame-Busting
**Vulnerability:** The anti-clickjacking frame-busting logic (`window.top.location = window.self.location`) was unhandled. If the page was embedded in an iframe with a restrictive sandbox policy lacking `allow-top-navigation`, attempting to read or write to `window.top` would throw a DOMException. This exception, if unhandled, could leak internal execution state and stack traces to the console or error reporting tools, which might be exposed to the attacker if they can monitor the error context.
**Learning:** Security mechanisms like frame-busting can inadvertently become information leak vectors themselves when interacting with browser security boundaries like HTML5 sandboxes.
**Prevention:** Always wrap frame-busting logic or cross-origin window access in a `try...catch` block to silently fail and prevent information leakage when blocked by browser security policies.
