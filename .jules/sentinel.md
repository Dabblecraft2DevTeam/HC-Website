...
## 2026-05-17 - Frame-busting DOMException Leakage
**Vulnerability:** Frame-busting logic (e.g., `window.top.location = window.self.location`) without a `try...catch` block can throw an unhandled `DOMException` when embedded in a highly restrictive iframe sandbox (without `allow-top-navigation`). This exception might expose stack traces or internal execution context.
**Learning:** When attempting to modify `window.top.location` for anti-clickjacking, we should anticipate access restrictions and sandbox constraints. The primary defense (the 'hide-first' pattern) will prevent clickjacking, but failing to catch exceptions from the secondary defense (frame-busting) leaks potentially sensitive execution context.
**Prevention:** Always wrap frame-busting logic in a `try...catch` block to handle potential `DOMException`s securely and fail gracefully without leaking stack traces.
