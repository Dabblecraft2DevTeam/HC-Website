...
## 2026-05-13 - Frame-Busting DOMException Leak
**Vulnerability:** The JavaScript frame-busting logic (`window.top.location = window.self.location`) was not wrapped in a `try...catch` block. When the site is embedded within a highly restrictive iframe (e.g., `sandbox` attribute without `allow-top-navigation`), attempting to access `window.top.location` throws a cross-origin `DOMException` which could leak stack traces or internal execution context.
**Learning:** When dealing with multi-origin interactions or potentially restrictive browser sandbox environments, operations like frame navigation can fail violently. Relying on an operation to just silently fail can unintentionally leak context or halt further script execution.
**Prevention:** Always wrap frame-busting logic (and similar cross-origin window manipulations) in a `try...catch` block. This ensures any `DOMException` is gracefully caught, preventing execution leaks and ensuring subsequent essential scripts can still execute.
