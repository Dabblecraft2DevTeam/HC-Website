...
## 2026-10-18 - Restricting Execution Contexts in CSP Meta Tags
**Vulnerability:** Even if a static site correctly restricts default, style, script, image, media, and object sources in its CSP meta tag, omitting restrictions on frames, child contexts, and web workers leaves potential execution vectors open if inline scripts or injected content attempt to instantiate them.
**Learning:** Defense-in-depth requires explicitly disabling unneeded execution capabilities. Even though `frame-ancestors` is unsupported in meta tags, directives like `frame-src 'none'`, `child-src 'none'`, and `worker-src 'none'` are perfectly valid and significantly reduce the attack surface by ensuring the static application cannot embed or spawn secondary execution contexts.
**Prevention:** Always include `frame-src 'none'; child-src 'none'; worker-src 'none';` in the Content Security Policy for static, non-interactive (or lightly interactive) sites that do not explicitly require framing or web workers.
