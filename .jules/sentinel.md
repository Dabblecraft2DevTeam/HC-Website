...
## 2024-05-24 - Restricting Child Contexts and Framing
**Vulnerability:** A static site that does not restrict its ability to embed frames, create child contexts, or spawn web workers exposes itself to potential vulnerabilities if an attacker manages to inject content or manipulate the DOM, increasing the attack surface.
**Learning:** While `frame-ancestors` cannot be enforced via a `<meta>` CSP tag to protect against being framed (clickjacking), defense-in-depth for a static site can be significantly improved by explicitly denying its own ability to embed frames (`frame-src 'none'`), create child browsing contexts (`child-src 'none'`), or spawn web workers (`worker-src 'none'`) within its own `<meta>` CSP tag.
**Prevention:** Always add `frame-src 'none'`, `child-src 'none'`, and `worker-src 'none'` to the CSP of static applications unless they explicitly require those features. This follows the principle of least privilege.
