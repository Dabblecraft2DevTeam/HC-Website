...
## 2024-06-26 - Missing Defense-in-Depth for CSP
**Vulnerability:** The static HTML files used a Content Security Policy via `<meta>` tags that lacked explicit restrictions for iframes (`frame-src`), child browsing contexts (`child-src`), and web workers (`worker-src`). Although `default-src 'self'` provides baseline protection, explicitly setting these directives to `'none'` improves defense-in-depth against advanced XSS or embedding attacks.
**Learning:** For a static site that doesn't intend to use iframes, child contexts, or web workers, it's a best practice to explicitly disable them via CSP. This prevents attackers from leveraging these vectors if they find a way to bypass the initial `default-src` restrictions.
**Prevention:** Always include `frame-src 'none'`, `child-src 'none'`, and `worker-src 'none'` in the Content Security Policy for static sites unless these features are explicitly required by the application's functionality.
