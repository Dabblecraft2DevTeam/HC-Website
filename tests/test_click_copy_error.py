...
def run():
    with sync_playwright() as p:
        ...
        # Grant clipboard permissions for testing, though we will mock it to fail
        ...
        page.add_init_script("""
            Object.defineProperty(navigator, 'clipboard', {
                value: {
                    writeText: () => Promise.reject(new Error("Simulated failure"))
                },
                writable: true,
                configurable: true
            });
        ...
        # Verify error states
        error_text = address_locator.text_content().strip()
        error_aria_label = address_locator.get_attribute("aria-label")
        error_title = address_locator.get_attribute("title")

        assert error_text == "Copy failed!", f"Expected 'Copy failed!', got {error_text}"
        assert error_aria_label == "Failed to copy server IP address", f"Expected 'Failed to copy server IP address', got {error_aria_label}"
        assert error_title == "Copy failed!", f"Expected 'Copy failed!', got {error_title}"
