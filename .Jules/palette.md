...
## 2026-05-19 - Hardcoding Initial ARIA States for Custom Navigation
**Learning:** Relying solely on JavaScript to inject initial ARIA states (like `aria-expanded="false"`) and `aria-controls` bindings into custom interactive elements like mobile menus causes a delay. Screen reader users may encounter the element before JS execution finishes, leading to a confusing or unannounced state.
**Action:** When creating custom navigation toggles (like mobile menus), always hardcode the initial `aria-expanded` state and explicit `aria-controls` bindings directly in the HTML source to ensure immediate screen reader accessibility and structural association before any JavaScript runs.
