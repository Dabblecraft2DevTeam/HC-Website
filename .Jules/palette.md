...
## 2026-06-14 - Pre-JS State Accessibility for Custom Toggles
**Learning:** When dealing with custom navigation toggles (like mobile menus), relying entirely on JavaScript to inject the initial `aria-expanded` state and explicit `aria-controls` bindings creates an inaccessible gap for screen reader users before the JavaScript executes or if it fails.
**Action:** Always hardcode the initial `aria-expanded` state and explicit `aria-controls` bindings directly in the HTML markup to ensure immediate accessibility before JavaScript execution.
