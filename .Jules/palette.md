...
## 2026-07-26 - Tactile Feedback for Transform Hover States
**Learning:** To improve micro-UX, always pair transform-based CSS `:hover` states (e.g., `scale` or `translateY`) with a complementary `:active` state (e.g., scaling down slightly or reducing translation) to provide continuous, satisfying tactile feedback during interactions.
**Action:** When adding transform-based `:hover` states to interactive elements, proactively include an `:active` state to provide immediate physical feedback upon interaction.
