...
## 2026-08-06 - Complementary Active States for Transforms
**Learning:** Elements that use CSS transforms (like `scale` or `translateY`) for `:hover` states feel floaty and lack tactile feedback when clicked if they don't have a corresponding `:active` state.
**Action:** Always pair transform-based `:hover` states with a complementary `:active` state (e.g., scaling down slightly or reducing the translation) to provide continuous, satisfying tactile feedback during interactions.
