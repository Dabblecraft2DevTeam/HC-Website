...
## 2026-06-12 - Hardcoding Initial ARIA States for Navigation Toggles
**Learning:** Initializing critical accessibility attributes like `aria-expanded="false"` and explicit controls bindings like `aria-controls="[id]"` via JavaScript `DOMContentLoaded` leaves screen reader users blind to the element's actual role and state during initial page load, before scripts are fully parsed and executed.
**Action:** When dealing with custom navigation toggles, hardcode the initial `aria-expanded` state and explicit `aria-controls` bindings directly in the HTML to ensure immediate accessibility, rather than relying purely on JS to inject them.
