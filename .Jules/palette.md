...
## 2026-06-13 - Hardcoding ARIA States for Custom Controls
**Learning:** When custom controls like mobile menu toggles rely entirely on JavaScript (`DOMContentLoaded`) to set their initial ARIA states (like `aria-expanded="false"` and `aria-controls`), screen reader users encounter incomplete semantics if they interact with the control before scripts load or if scripts fail.
**Action:** Always hardcode initial ARIA states directly in the HTML markup for custom controls, ensuring immediate and robust screen reader accessibility independent of JavaScript execution timing.
